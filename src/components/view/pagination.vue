            ...
            >
                ...
                <el-table-column
                    ...
                    <template slot-scope="scope">
                        <span v-if="options.itemVal[index].split(',').length == 1">{{ scope.row[options.itemVal[index]]
                            }}</span>
                        <span v-else>
                ...
                <el-table-column
                    resizable
                    v-for="(item, index) in options.itemTr" :key="tableKey"
                    v-if="typeof options.itemVal[index] == 'object'"
                    :width="options.itemVal[index].width ? options.itemVal[index].width : options.itemVal[index].data.length > 1 ? '220': '140'"
                    :label="item">
                    <template slot-scope="scope">
    ...
    export default {
        ...
        methods: {
            ...
            showBtn(keyItem, item){
                if (keyItem.showItem === undefined) {
                    return true
                } else {
                    return keyItem.showItem(item)
                }
            },
            showItem(childList, item){
                if (childList.showItem === undefined) {
                    return true
                } else {
                    return childList.showItem(item)
                }
            },
